...
import { apiClient, ApiResponse } from "../../shared/utils/api";
...
const renderCell = (val: any, _type: string) => {
  if (val === null) return <span class="text-gray-400 dark:text-slate-500 italic block truncate">{RESOURCES.SQL.NULL_VALUE}</span>;
...
export default function SqlPage() {
  ...
  const fetchPage = async (page: number, size: number) => {
    ...
    showLoading("database", RESOURCES.SQL.EXECUTING_QUERY);
    try {
      ...
      setErrorResponse({
        status: "ERROR",
        message: err.message || RESOURCES.SQL.FAILED_TO_EXECUTE,
        details: err
      } as any);
    } finally {
      setIsLoading(false);
  ...
  return (
    <main class={`mx-auto px-4 py-6 transition-all duration-300 w-full ${isWideMode() ? 'max-w-[95%]' : 'max-w-5xl'}`}>
      ...
      >
        <h1 class="text-5xl font-extrabold bg-clip-text text-transparent bg-gradient-to-r from-primary-600 to-secondary-500 mb-2">{RESOURCES.SQL.TITLE}</h1>
        <p class="text-gray-500 dark:text-slate-400 text-lg">{RESOURCES.SQL.SUBTITLE}</p>
      </Motion.div>
      ...
      <div class="w-full">
        ...
        >
          <div class="flex flex-col gap-1">
            <div class="flex items-center justify-between px-1">
              <label class="block text-sm font-black text-slate-700 dark:text-slate-300 uppercase tracking-wider">{RESOURCES.SQL.QUERY_LABEL}</label>
              <span class="text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-widest bg-slate-100 dark:bg-slate-800/50 px-2 py-1 rounded-md border border-slate-200 dark:border-slate-700/50">{RESOURCES.SQL.SHORTCUT_HINT}</span>
            </div>
            <textarea
          ...
          <div class="flex gap-3 justify-center">
            <TextButton type="submit" variant="solid" size="sm" class="shadow-md" disabled={isLoading()} icon={<TbOutlinePlayerPlay size={16} />}>
              {isLoading() ? RESOURCES.SQL.EXECUTING : RESOURCES.SQL.EXECUTE}
            </TextButton>
          ...
          <Presence exitBeforeEnter>
            <Show when={runCount()}>
              {(_) => (
                ...
                >
                  <Show when={result()?.columns?.length > 0} fallback={<div class="p-6 text-center text-gray-500 dark:text-slate-400">{RESOURCES.SQL.NO_RESULTS}</div>}>
                    <DataTable
        ...
        <DataModal
          ...
          title={RESOURCES.SQL.ERROR_DETAILS_TITLE}
