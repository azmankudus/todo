    ...
    <div class={`w-full mx-auto px-6 py-12 transition-all duration-300 ${isWideMode() ? 'max-w-[98%]' : 'max-w-6xl'}`}>
      ...
      >
        <h1 class="text-5xl font-black text-slate-900 dark:text-white tracking-tight">
          Error <span class="text-transparent bg-clip-text bg-gradient-to-r from-primary-600 to-secondary-500">Showcase</span>
        </h1>

        <div class="mt-6 flex flex-col items-center gap-4">
          <p class="text-slate-500 dark:text-slate-400 font-medium">
            Select a system failure signature to simulate global modal feedback.
          </p>
          ...
          >
            ...
            Backend API: {errorStore.useBackend() ? 'Online' : 'Offline Mode'}
          </button>
        </div>
      </Motion.div>
        ...
        <For each={options}>
          {(code, i) => (
            <Motion.button
              ...
              press={{ scale: 0.98 }}
              onClick={async () => {
                showLoading("hourglass", "Synchronizing failure signature...");
